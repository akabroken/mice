...
router.get('/', async function (req, res, next) {
  try {
   ...
   // res.send(htmlContent); // Send the HTML content as a response
    var sql_query = 'SELECT e.eventId, e.eventName, CONCAT_WS(" ", c.firstName, c.lastName) AS customer, e.eventdate, b.status, b.totalPrice FROM mice_tbl_events e INNER JOIN mice_tbl_customer c ON c.customerId = e.customerId INNER JOIN mice_tbl_bookings b ON b.eventId = e.eventId ORDER BY b.bookingId DESC';

    // dbConn.query(sql_query, function (err, rows) {
    //   if (err) {
    //     req.flash('error', err);
    //   } else {
    //     res.render('dashboard/dashboard', { title: 'Admin Dashboard', menuItems, data: rows });
    //   }
    // });
    res.render('dashboard/dashboard', { title: 'Admin Dashboard', menuItems,events,formatDate, page: page,
    pageSize: pageSize, });
